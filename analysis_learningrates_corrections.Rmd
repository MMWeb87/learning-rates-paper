...
# Results for PV -- in lead currency `r params$lead_currency` & with `r params$capacity_data` marketshare 
...
combined_plot_data <- average_global_costs_real %>% 
  inner_join(X) %>% 
...
# for debugging puropses. Not used for lr calculation
alpha <- left_join(P$P_nominal, P_tilde$P_nominal, by = c("currency", "year"), suffix = c("", "_tilde")) %>% 
  mutate(alpha = costs_tilde / costs)
  
#write_debug_information("alphas")


```

Here we test if the propositions make sense. To test the propositions, run the file with params run_test:yes
```{r Test of prop 2}

if(params$run_test){
  
  # check prop 2
  # in test file, EUR is i, and all others are l
  # alphas > 2006 must be > 1, and P_tilde > 2006, > P
  alpha %>% View()
}
...
for(type in types){
  ...
  for(i in 1:length(intervals)){
    ...
    write_debug_information(l$b1, paste0(debug_prefix, "coefficient_log"))
...
corrected_rates <- results$corrected_learning_rates %>% 
  filter(type == "corrected") %>%
  gather(key = "interval", value = "corrected_lr", contains("-")) %>%
  select(corrected_lr, interval)
...
if(!params$run_test){
  ...
  bind_rows(lr_result_1,lr_result_2) %>% 
    write_csv(paste0("output/results/learning_rate_results_test_",params$capacity_data,"_",params$lead_currency,".csv"))
