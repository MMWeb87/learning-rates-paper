...
P_comp_tilde_sum <- P_comp_tilde %>% 
  mutate(real_costs = nominal_costs / defl) %>% 
  group_by(year, lead_currency) %>% 
  summarise(P_nominal = sum(nominal_costs), P_real = sum(real_costs))

P <- select(P_comp_sum, currency = lead_currency, year, costs = P_nominal)
P_real <- select(P_comp_sum, currency = lead_currency, year, costs = P_real)
P_tilde <- select(P_comp_tilde_sum, currency = lead_currency, year, costs = P_nominal)
P_tilde_real <- select(P_comp_tilde_sum, currency = lead_currency, year, costs = P_real)
