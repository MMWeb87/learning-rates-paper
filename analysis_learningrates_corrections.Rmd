...
relevant_projects <- filter(commissioned_projects,
    ...
    !is.na(date)) %>% 
  mutate(subset_name = "BNEF relevant")
...
projects_with_costs_in_interval <- filter(projects_with_costs,
  ...
  mutate(subset_name = "BNEF with costs in interval")
...
summary_table[6,]  <- list("all relevant with costs in interval", 
...
print_number_table(table = summary_table, caption = "Summary of dataset. The share relates to the commissioned", digits = 2)
...
irena_data <- read_csv("input/irena-capacity-generation.csv") %>% 
  ...
  add_column(subset_name = "IRENA all")
...
x_global %>% 
  ...
  filter(year <2010) %>% 
  ggplot(mapping = aes(x = year, y = X)) +
    geom_line(mapping = aes(linetype = subset_name, colour = subset_name)) 
           
X <- x_global[[params$x_global_for_cumsum]] %>%
  mutate(X = cumsum(x_global)) %>% 
  select(year, X)

if(params$X_norm){
  normyear_value <- filter(X, year == T0) %>% pull(X)
  X <- X %>% 
