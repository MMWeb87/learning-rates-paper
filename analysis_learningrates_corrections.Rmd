...
We chose `r length(relevant_currencies)` relevant currencies (`r currencies_print`) and calculate a learning rate between `r T0` and `r T_max`. We look only at PV projects above `r params$capacity_threshold` MW. For the exchange rate conversion, we chose the month (`r params$exchangerate_reference_month`) as reference month.
...
projects_in_interval <- filter(data_projects,
  between(year, T0, T_max))

commissioned_projects <- filter(projects_in_interval, 
    status == "Commissioned")

relevant_projects <- filter(commissioned_projects,
...
stats[["number"]][["in_interval_projects"]] <- nrow(projects_in_interval)
stats[["capacity"]][["in_interval_projects"]] <- sum(projects_in_interval$capacity, na.rm = T)
...
if(params$debug){
  ...
  rm(data_projects, projects_in_interval, commissioned_projects, relevant_projects, non_relevant_projects1, non_relevant_projects2, projects_with_costs_with_outliers, relevant_projects_completed)
}
...
summary_table[2,] <- list(paste("all between", T0, "and",T_max), 
                          stats[["number"]][["in_interval_projects"]], 
                          NA, 
                          stats[["capacity"]][["in_interval_projects"]], 
                          NA)

summary_table[3,]  <- list("all commissioned", stats[["number"]][["commissioned_projects"]], 1, stats[["capacity"]][["commissioned_projects"]], 1)

summary_table[4,]  <- list("all relevant", 
...
summary_table[5,]  <- list("all relevant with costs and outliers", 
...
summary_table[6,]  <- list("all relevant with costs", 
...
print_number_table(table = summary_table, caption = "Summary of dataset. The share relates to the commissioned")
...
2. For each project, we calculate the value of the lead currency (`r params$lead_currency`) from the local value and an exchange rate that is either close to the project date or fixed for each year (if always_use_fixed_er == yes). To be consistent with method 2, we use a fixed exchange rate per year in our paper.
...
print_number_table(rbind(simple_learning_rates,"sd"=simple_learning_rates_sd), "Simple learning rates in all currencies. The SD describes the variation between the different learning rates." )
print_number_table(simple_learning_rates_rsquared, "R-squared values of simple learning rates")
...
P <- tibble(
  ...
  "year" = all_years,
  "nominal_costs" = numeric(length(all_years))
)
...
for(t in all_years){
  ...
  for(i in relevant_currencies){
    ...
    P[P$year == t, "nominal_costs"] <- P[P$year == t, "nominal_costs"] + delta_i_t * w_i_t * C_i_t
    P_tilde[P_tilde$year == t, "nominal_costs"] <- P_tilde[P_tilde$year == t, "nominal_costs"] + delta_i_t * w_i_0 * C_i_t

  }
  
  alphas[alphas$year == t, "alpha"] <- P_tilde[P_tilde$year == t, "nominal_costs"]  / P[P$year == t, "nominal_costs"]

}
```

### Step 4: Convert to real values

```{r}
lead_currency_defl <- filter(defl, currency == lead_currency)
P_real <- convert_to_real_costs(P, lead_currency_defl)
P_tilde_real <- convert_to_real_costs(P_tilde, lead_currency_defl)
```
...
Plotting all learning rates in the same plot, it is necessary to norm the costs. To do this, we select one random project and devide all average global costs by that project's value. We then get 1/W for all average costs and can compare the learning rates. The norm project is the following.

```{r }

uncorrected_learning_rates <- average_global_costs_real %>% 
  inner_join(X) %>% 
  select(
    year, 
    average_costs = real_costs, 
    currency, 
    cumulative_capacity = X) %>% 
  add_column(type = "uncorrected")

corrected_learning_rates <- select(corrected_learning_rates_data_list$adjusted[[combined_plot_interval]],
  year = years, 
  average_costs = costs, 
  currency = lead_currency, 
...
# To be consistent with the paper, I change the naming of the types here
combined_plot_data$type <- factor(combined_plot_data$type, labels = c("corrected", "uncorrected"))
...
A combined plot over the interval of: `R params$combined_plot_interval`.
...
ggplot(combined_plot_data_normed, aes(cumulative_capacity/1000, normed_average_costs)) +
  geom_point(aes(col=currency, size=type)) +
  geom_smooth(data=. %>% filter(type=="uncorrected"), 
    aes(col=currency, linetype = currency, size=type), 
    method="lm", formula = (y ~ x), se=FALSE) +
  geom_smooth(data=. %>% filter(type=="corrected"), 
    linetype = "solid", aes(size=type), 
    method="lm", formula = (y ~ x), se=FALSE, col="black") +
  
  scale_linetype_manual(values = plot_order_linetype) +
  scale_size_manual(values = c("uncorrected" = 0.6, "corrected" = 1.6)) +
  ...
  labs(x = "Cumulative capacity [GW]", 
       y = paste0("Index of average yearly 2017-costs per MW"),
