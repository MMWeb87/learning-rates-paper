...
```{r Load BNEF data, echo=FALSE, warning=FALSE,}

exchangerate_reference_month <- 1

# Use the nominal values of PV and defalte afterwards. Use the financing date

setClass('myDate')
setAs("character","myDate", function(from) as.Date(from, format="%Y-%m-%d") )

data_all <- read.csv("input/data_PV_all_nominal.csv", 
                     colClasses = c("Financing.Date"="myDate"), 
                     na.strings=c("","NA"),
                     stringsAsFactors = F)

data_projects <- select(
  data_all,
  id = X...Renewable.Project.ID,
...
```{r Complete currencies in BNEF, echo=FALSE, warning=FALSE, }

# load the currency translation tables and merge them with the data to get the corresponding entries.
country_to_currency_translation <- read.csv("input/country_to_currency_translation.csv", stringsAsFactors = F)
currency_to_currency_area_translation <- read.csv("input/currency_to_currency_area_translation.csv", stringsAsFactors = F)
currency_translation <- read.csv("input/currency_translation.csv", stringsAsFactors = F)

country_short_to_currency <- data.frame(currency = country_to_currency_translation$currency)
rownames(country_short_to_currency) <- country_to_currency_translation$country_short

currency_to_currency_area_translation <- select(currency_to_currency_area_translation, currency = "X...currency", currency_area)

relevant_projects <- merge(
  relevant_projects, 
  select(currency_translation, local_currency = short, local_currency_long = long), 
  by = "local_currency_long", 
  all.x = T)

relevant_projects <- merge(
  relevant_projects, 
  select(country_to_currency_translation, country = country_long, local_currency_added = currency), 
  by = "country", 
  all.x = T)

# replace the NA data in the local_currency column (from the original local_currency info) with the local_currency_added data (from assuming the total_local_value was the main currency in the repsective country)
relevant_projects$local_currency[is.na(relevant_projects$local_currency)] <- relevant_projects$local_currency_added[is.na(relevant_projects$local_currency)]

# Select final df and add year
relevant_projects <- select(relevant_projects, date, capacity, total_local_value, local_currency, id)
relevant_projects$local_currency <- as.factor(relevant_projects$local_currency)
relevant_projects$year <- year(relevant_projects$date)

## data to be used
# final data only the ones with costs
data_with_costs_pre <- filter(
  relevant_projects,
  !is.na(total_local_value),
  !is.na(local_currency),
  local_currency != "Other"
)

# Transform costs from Million currency to specific currency/W
data_with_costs_pre <- transform(data_with_costs_pre, local_value = total_local_value / capacity)
data_with_costs_pre <- select(data_with_costs_pre, date, capacity, local_value, local_currency, year, id)

# Final selection of data for the market shares (ie. no local value)
bnef_data <- select(relevant_projects, date, capacity, local_currency, year)
...
cpi_deflators <- select(cpi_deflators, country = X...LOCATION, year = TIME, value = Value)
...
exchange_rates_USD <- select(exchange_rates_pre,date = X...Date, 
                         EUR = EUR.USD, 
                         GBP = GBP.USD, 
                         CHF = CHF.USD, 
                         INR = IND.USD,
...
In Table 3 & 4 and Figure 2, we present the results of the simple learning rate approach.
